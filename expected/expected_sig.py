...
def spec_dens_Evans_for_plot(temp_elec_ev: float, temp_ions_ev: float, wavelen_scat_nm: float,
    ...
    debay = (temp_elec_ev * all_const['ev_to_erg'] / (4 * math.pi * n_e_cm * all_const['q_e'] ** 2)) ** (
            1 / 2)  # centimeters
    ...
    omega_e = 2 * omega * (2 * temp_elec_ev * all_const['ev_to_erg'] / all_const['m_e']) ** 0.5 * math.sin(
        theta_rad / 2) / all_const['c_light']
    omega_i = 2 * omega * (2 * temp_ions_ev * all_const['ev_to_erg'] / all_const['m_ion']) ** 0.5 * math.sin(
        theta_rad / 2) / all_const['c_light']
...
def section_Evans(temp_elec_ev: float, temp_ions_ev: float, wavelen_scat_nm: float, wavelen_incident_nm: float,
    ...
    #                                               !!!!!CГС!!!!
    # спектр взят с Пятницкого стр 165. (6.2) - у него из Эванса
    with open('D:\Ioffe\TS\divertor_thomson\different_calcuations_py\spectre_for_low_T\expected\constants',
              'r') as file:
        all_const = json.load(file)
    ...
    omega_e = 2 * omega * (2 * temp_elec_ev * all_const['ev_to_erg'] / all_const['m_e']) ** (1 / 2) * math.sin(
        theta_rad / 2) / all_const['c_light']
    omega_i = 2 * omega * (2 * temp_ions_ev * all_const['ev_to_erg'] / all_const['m_ion']) ** (1 / 2) * math.sin(
        theta_rad / 2) / all_const['c_light']
    ...
    const = all_const['q_e'] ** 4 / (
            all_const['m_e'] ** 2 * all_const['c_light'] ** 4 * math.pi ** (1 / 2)) * 1E-4  # 1E-4 - to meters^2

    # ions_input_cons = (temp_elec_ev * all_const['m_ion'] / (temp_ions_ev * all_const['m_e']) )**0.5

    sigma = const * (
            Gamma(alpha, x) / omega_e + z_eff * (alpha ** 2 / (1 + alpha ** 2)) ** 2 * Gamma(beta, y) / omega_i)
...
def load_detector_data(detector_path: str) -> (list, list):
    ...
    for i in range(2, len(detector_file)):
        ...
        detector_phel.append(
            float(detector_file[i].split(',')[1]) / 100)  # данные в файле - в процентах, поэтому /100 !
...
def build_poly(wl_grid: list, all_ch_trans: list, detector_wl: list, detector_phel: list):
    ...
    for ch_num in range(len(all_ch_trans)):
        ...
        for wl_ind in range(len(wl_grid)):
            ...
            detector = detector_phel[filter_ind] + (detector_phel[filter_ind + 1] - detector_phel[filter_ind]) / (
...
if __name__ == '__main__':
    ...
    filters_data_wl, filters_data_trans, chan_num = load_filter_data('filters_equator.csv')
    ...
    for wl in wl_grid_nm:
        break
        print('%.5e %.4e ' % (wl, section_Evans(0.3, 0.3, wl, 1064.4, 130, 1E21,
                                                zeff)))  # sigma / (omega_frequency * omega_scat) или же сечение на такой частоте в такой телесный угол) m^2 * s / st
